...
import { fetchPaymentReceivalById, createPaymentReceive } from "@/slices/paymentSlice";
...
const PaymentReceiveForm = ({ title, receiptNo, handleComponentChange }) => {
            ...
            <div className="d-flex align-items-center gap-2">
              {isEditing ? (
                <Button
                  type="button"
                  variant="primary"
                  onClick={() => tableRef.current?.saveRow(rowIndex)}
                >
              ...
              <Button
                type="button"
                variant="danger"
                onClick={() => tableRef.current?.deleteRow(rowIndex)}
              >
  ...
  const submitHandler = async (data) => {
    ...
    if(title === "Create Payment Receive"){
        dispatch(createPaymentReceive(payload));
        formResetHandler();
    }
    // 
  };
    ...
    >
      {title === "Modify Receipt" && (
        <div className="d-flex justify-content-start align-items-center gap-2">
          ...
          <Button type="button" variant="primary" title="Search" onClick={searchHandler}>
            Search
          </Button>
        </div>
      )}
        ...
        {fronFields.map((fronField) => (
          <div className={fronField.container} key={fronField.name}>
            <Controller
              name={fronField.name}
              control={control}
            //   rules={isNotValidation(fronField.name) ? {} : fronField?.rules ?? {}}
              render={({ field, fieldState }) => {
                ...
                const shouldDisable =
                  ...
                  fieldsToDisableForCash.includes(fronField.name);
                ...
                if (fronField.type === "select" || fronField.type === "radio") {
                  ...
                  if (fronField.name === "receiveMode") {
                    options = ["Cash", "UPI", "Bank Transfer", "Cheque", "RTGS"].map(
                      (el) => ({
                        value: el,
                        label: el,
                      })
                  ...
                  if (fronField.name === "currency") {
                    options = ["INR", "USD", "EUR", "GBP", "JPY", "CNY"].map(
                      (el) => ({
                        value: el,
                        label: el,
                      })
