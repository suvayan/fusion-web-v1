...
import { InputField, TextareaField, SelectField, RadioGroupField, DateField } from "@/components/ui/inputs/InputFields";
import EditableTable from "@/components/ui/table/EditableTable";
import {fetchCountries, fetchDepositBanks, fetchConcernDepartments, fetchDepositBankAccounts} from "@/slices/commonSlice";
import { fetchPaymentReceivalById} from "@/slices/paymentSlice";
...
const PaymentReceiveForm = ({title, receiptNo, handleComponentChange}) => {
    ...
    const isBack = title !== "Modify Receipt" ? true : false;
    const backTitle = title !== "Modify Receipt" ? "Back to Payment Receive List" : "";
    const {countries, depositBanks, depositBankAccounts, concernDepartments} = useSelector((state) => state.common);
    const { paymentReceiveById } = useSelector((state) => state.payment);
    ...
    const columns = useMemo(() => [
        ...
        },
        {
            ...
            type: "",
            render: (props) => {
                const {rowIndex, editRowIndex} = props;
                const isEditing = editRowIndex === rowIndex;
                return(
                    <div className="d-flex align-items-center gap-2">
                        {isEditing ? (
                            <Button variant="primary" onClick={() => tableRef.current?.saveRow(rowIndex)}>
                                <Check size={16} strokeWidth={2} />
                            </Button>
                        ):(
                            <Button 
                                variant="success"
                                onClick={() => tableRef.current?.editRow(rowIndex)}
                            >
                                <Pen size={16} strokeWidth={2} />
                            </Button>
                        )}
                        <Button variant="danger" onClick={() => tableRef.current?.deleteRow(rowIndex)}>
    ...
    const isNotValidation = useCallback((arg)=>{
        const receiveMode = formValues?.receiveMode ?? "";
        const fieldsToDisableForCash = ["ddChqNo", "ddChqDate", "bankName", "branchName", "depositBank", "depositBankAcNo"];
        if(receiveMode === "Cash"){
            return fieldsToDisableForCash.includes(arg);
        }
        return false
    ...
    const submitHandler = async (data) => {
        ...
        if(items.length) {
            const sum = items.reduce((acc, item) => acc + Number(item.ri_amount || 0), 0);
            if(Number(data.amount) !== sum) {
                setErr("Total of Received Amount in items should be equal to Amount field");
                return;
            }else{
                setErr("");
            }
    ...
    return (
        <FormWrapper 
            ...
            isBack={isBack}
            backTitle={backTitle}
            handleBack={()=> typeof handleComponentChange === "function" && handleComponentChange("view")}
        >
            {title === "Modify Receipt" && (
                <div className="d-flex justify-content-start align-items-center gap-2">
                    <span className="fw-semibold text-dark font-14">Receipt No.</span>
                    <InputField 
                        className="w-25"
                        id = "receiptInput"
                        value={receiptInput}
            ...
            <form className="row p-3 g-3" onSubmit={handleSubmit(submitHandler)}>
                {fronFields.map((fronField) => (
                    <div className={fronField.container} key={fronField.name}>
                        <Controller 
                            ...
                            control={control}
                            rules={
                                isNotValidation(fronField.name)? {} : fronField?.rules ?? {}
                            }
                            render={({field, fieldState})=>{
                                const fieldsToDisableForCash = ["ddChqNo", "ddChqDate", "bankName", "branchName", "depositBank", "depositBankAcNo"];
                                const shouldDisable = formValues?.receiveMode === "Cash" && fieldsToDisableForCash.includes(fronField.name);
                                fronField.disabled = shouldDisable;

                                let options = [];
                                if(fronField.type === "select" || fronField.type === "radio") {
                                    ...
                                    if(fronField.name === "depositBank") {
                                        options = depositBanks?.map((bank) => ({
                                            value: bank.bm_id,
                                            label: bank.bm_name
                                        })) || [];
                                    }
                                    if(fronField.name === "depositBankAcNo"){
                                        options = depositBankAccounts?.map((ac) => ({
                                            value: ac.bmd_ac_no,
                                            label: ac.name
                                        })) || [];
                                    }
                                    ...
                                    if(fronField.name === "localOutStation"){
                                        options = ["Local", "Outstation"].map((el) => ({
                                            value: el,
                                            label: el
                                        })) || [];
                                    }
                                ...
                                return (
                                    <InputField 
                                        label={fronField.name !== "receiptNo" ? fronField.label : receipt? fronField.label : null}
                                        id={fronField.name}
                                        type={fronField.name !== "receiptNo" ? fronField.type : receipt? "text" : "hidden"}
                                        readOnly={fronField.readOnly}
                                        disabled={fronField.disabled}
                                        {...field}
                                        error={fieldState?.error?.message}
                ...
                <div className="col-md-12 col-sm-12">
                    <span className="text-muted cursor-pointer d-flex align-items-center gap-2" onClick={() => tableRef.current?.addRow()}>
                ...
                <div className="col-md-12 col-sm-12 d-flex justify-content-center gap-2">
                    ...
                    <Button type="button" variant="danger" title="Reset" onClick={formResetHandler}>Reset</Button>
                </div>
            </form>
        </FormWrapper>
    )
